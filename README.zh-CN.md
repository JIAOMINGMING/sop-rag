...
只需要 Python 3 和一个 OpenAI API key（向量化＋作答共用一个 key；充 5 美元能用很久——建库不到 1 分钱、每次提问约 0.001 美元）：

```bash
git clone https://github.com/JIAOMINGMING/sop-rag.git && cd sop-rag
./install.sh        # 装依赖 + 引导填 key + 首次建索引
```

## 使用（三种入口）

- **普通用户（推荐）**：双击 `start_web.command`，浏览器自动打开一个搜索框页面——输入问题，回答带可点击的出处链接（PDF 直落那一页），还能展开看检索命中的原文片段。服务只监听本机 `127.0.0.1`，文档不出电脑（仅 LLM API 调用）。
- **终端用户**：
  ```bash
  .venv/bin/python ask.py "偏差的初步评估要在几天内完成？"
  .venv/bin/python ask.py --show-hits "..."   # 同时显示检索命中
  ```
- **文档更新**：把文件丢进 `docs/` 即可——macOS 上 launchd 监听自动增量入库（或手动跑 `.venv/bin/python ingest.py`）。

作答默认走 OpenAI（`gpt-4o-mini`，可用 `SOP_RAG_CHAT_MODEL` 换模型），**不需要 Claude 订阅**；想用 Claude 作答的装 Claude Code 后设 `SOP_RAG_LLM=claude`，没填 OpenAI key 时也会自动回退到 claude 命令行。

OPENAI_API_KEY 读取顺序：环境变量 → 项目内 `.env` → `~/learning-plan/coach/config.sh`。
...
## 更想要「配方」而不是代码？用 skill

本仓库同时附带一份蒸馏好的方法论 skill（[`skill/document-rag/SKILL.md`](skill/document-rag/SKILL.md)）：

- **Claude Code 用户**：`cp -r skill/document-rag ~/.claude/skills/`，然后一句话「用 document-rag skill 给 ./我的文档 建个问答库」即可自动唤起。
- **非 Claude Code 用户**：skill 就是纯 Markdown 配方，只有自动触发是 Claude Code 特有的——Codex CLI 贴进 `AGENTS.md`、Copilot 放 `.github/copilot-instructions.md`、Cursor 放 `.cursor/rules/`，或直接把文件粘给任何 AI 说「照这个配方在我的文档上搭一套」。
